 ...
 * {@link ClubAnnouncementCtx}. Browser callers omit it and get the shared
 ...
 * Unwrap every shape to a plain record. Same defence as `@/lib/focus`.
 */
...
function requireBody(body: string): string {
  ...
  if (trimmed.length < ANNOUNCEMENT_BODY_MIN) {
    throw new ClubAnnouncementError("Write something for your members to read.");
  }
 ...
 */
async function requireUserId(ctx?: ClubAnnouncementCtx): Promise<string> {
 ...
 * nobody clicks one that RLS will quietly ignore.
 *
