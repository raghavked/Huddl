 ...
 *
 * That second half is the one failure students will actually hit: opening
 ...
 * {@link StudyBuddyCtx}. Browser callers omit it and get the shared browser
 ...
 * textarea here so the counter and the check constraint agree.
 */
 ...
 * shapes to a plain record. Same defence as `@/lib/focus`.
 */
 ...
 */
async function requireUserId(ctx?: StudyBuddyCtx): Promise<string> {
 ...
 */
export async function countBuddies(
  courseId: string,
  ctx?: StudyBuddyCtx
): Promise<number> {
 ...
 *
 * Call it on load to decide between "I'm looking too" and the editable card
 * showing their note with a way to take it down.
 *
 * @param courseId `courses.id` to check.
 * @throws {StudyBuddyError} With copy that's ready to render.
 */
export async function fetchMyOptIn(
  courseId: string,
  ctx?: StudyBuddyCtx
): Promise<MyOptIn | null> {
 ...
 * in the browser first, so the 140-character constraint is a backstop rather
 ...
 * intent ("I'm not looking") already holds, and a second click on a stale
 * page shouldn't produce an error.
 *
