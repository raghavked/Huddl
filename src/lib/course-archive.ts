 ...
 * query and splits them here: a student has a couple of dozen enrollments at
 * the very most, and one round trip means the courses page can say "4
 * shelved" without a second request.
 *
 * WHERE THIS GETS READ FROM
 * Every other surface that lists classes — the study plan, the calendar,
 * home, the course pickers in focus sessions and grades — should read
 * **active courses only**. That is the entire point of the feature: a
 * finished class stops crowding this quarter.
 *
 ...
 *
 * No React, no theme, no routing in here — queries, narrowing, and one pure
 * grouping helper. Every failure arrives as a {@link CourseArchiveError}
 ...
 * {@link CourseArchiveCtx}. Browser callers omit it and get the shared
 * browser client; a server component passes its request-scoped client (and
 * the user id it already resolved) so nothing reads the session cookie twice.
 */
 ...
 * exact shape a row on the courses page renders, on either shelf.
 *
...
export type MyCourse = {
  ...
  /**
   * How the class got here. Web-only addition to the native shape: the
   * courses page draws a provenance line under every card, and carrying it
   * on this row is what keeps that page down to a single query.
   */
...
export const MY_COURSES_SELECT =
  "id, source, archived_at, course:courses(id, code, title, term:terms(name, starts_on))";
...
function db(ctx?: CourseArchiveCtx): Db {
...
/**
 * The enrollment's source, defaulting to "manual". The retired import flows
 * left 'canvas' and 'schedule_image' rows behind, and an unknown value is
 * never a reason to lose the course off the page.
 */
function toSource(raw: unknown): EnrollmentSource {
  return raw === "canvas" || raw === "schedule_image" || raw === "manual"
    ? raw
    : "manual";
}
 ...
 */
async function requireUserId(ctx?: CourseArchiveCtx): Promise<string> {
 ...
 *
 * One query, split here. The SELECT policy on `enrollments` also lets a
 * student see their classmates' rows, so this filters on `user_id`
 ...
 * empty state inviting them to add their first course, and the courses page
...
async function setArchivedAt(
  ...
  // (dropped in another tab). Either way nothing moved.
 ...
 * than failing, so a second click on a stale page is harmless.
 *
...
export async function unarchiveCourse(
  ...
  await setArchivedAt(enrollmentId, null, UNSHELVE_FAILED, ctx);
}
 ...
 * means the shelf is empty and the page should show its empty state.
 *
