 ...
 *
 * The one web-only adjustment: {@link fetchSemester} takes an optional
 * trailing {@link SemesterCtx}. Browser callers omit it and get the shared
...
/** Everything the semester page needs, in one shot. See {@link fetchSemester}. */
 ...
 * table of cutoffs, which is the whole point: a page showing 90% → "A-" and
 ...
 * quarter you look. `graded` and `total` report the gap out loud so a page can
 * say "3 of 5 classes" beside the number, and `note` says it in a sentence.
 *
...
export function semesterEstimate(
  ...
  // belt and braces, because a NaN reaching a page as someone's GPA is the one
  // failure mode worth two lines of paranoia.
  const gpa =
 ...
 * page decides its own placeholder rather than inheriting one from a library.
 *
...
function db(ctx?: SemesterCtx): Db {
 ...
 */
async function requireUserId(ctx?: SemesterCtx): Promise<string> {
 ...
 * would cost a student with eight classes eight round trips on a page that
 ...
 *
 * Then the grouping and the estimating happen here, through the same
 * {@link courseEstimate} the single-course grade page uses, so a class reads
 ...
 */
export async function fetchSemester(ctx?: SemesterCtx): Promise<Semester> {
  const userId = await requireUserId(ctx);

  const { data: enrollmentRows, error: enrollmentError } = await db(ctx)
    .from("enrollments")
...
export async function setCourseUnits(
  ...
  const { error } = await db(ctx).rpc("set_course_units", {
