 ...
 * {@link ReminderCtx}. Browser callers omit it and get the shared browser
...
function db(ctx?: ReminderCtx): Db {
...
/**
 * Check a lead against the window the column enforces, so a bad value fails in
 * the browser with a sentence instead of as a constraint violation after a
 * round trip. Values from {@link LEAD_CHOICES} always pass.
 *
 ...
 */
async function requireUserId(ctx?: ReminderCtx): Promise<string> {
 ...
 *
 * Nothing here is scheduled in the browser. The row is the whole request; the
 * hourly sweep does the sending, so this keeps working with the tab closed.
 *
...
export async function setReminder(
  ...
  // Checked before the session lookup: a bad lead is a bug in the client, and
 ...
 *
 * Pure: `now` is passed in, never read from the clock, so a ticking page and a
 * unit test get the same answer.
 *
