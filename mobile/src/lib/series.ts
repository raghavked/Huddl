...
export function generateSeries(spec: SeriesSpec): CalendarRowDraft[] {
  ...
  // on the phone rather than something to write to the calendar.
 ...
 * confirm, a test hands over a fixed moment, and both get the same rule.
 *
...
export async function updateSeries(
  ...
  if (patch.details !== undefined) fields["details"] = optionalText(patch.details);
 ...
 * intent already holds, and a second tap on a stale screen shouldn't be an
 * error.
 *
 * @param seriesId The shared id from {@link SeriesRow.series_id}.
