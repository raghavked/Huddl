 ...
 *
 * Same module, two homes: the web app carries this file's twin at
 * src/lib/friends.ts, where every I/O function takes an optional context so
 * server components can hand in their request-scoped client. Native has one
 * client for the whole app, the `@/lib/supabase` singleton, so everything
 * here just uses it, the way `@/lib/blocks` and `@/lib/group-dm` do.
 */
...
/**
 * One friend edge, whoever asked first. Mirrors the web's `Friendship` in
 * `src/lib/types.ts`: the pair (requester_id, addressee_id) is the primary
 * key, and there is never a mirror row for the other direction.
 */
 ...
 * Unwrap every shape to a plain record. Same defence as `@/lib/group-dm`.
 */
...
/**
 * The caller's `profiles.id`, read from the stored session. Null rather than
 * a throw when nobody is signed in: every caller here has a warmer answer
 * for that than an error.
 */
 ...
 * @throws {FriendsError} With copy that's ready to render.
 */
export async function listFriendships(): Promise<FriendEdgeRow[]> {
  const myId = await currentUserId();
  if (myId === null) return [];
 ...
 * @throws {FriendsError} With copy that's ready to render.
 */
export async function sendRequest(otherId: string): Promise<void> {
  const myId = await currentUserId();
  if (myId === null) {
 ...
 * @throws {FriendsError} With copy that's ready to render.
 */
export async function acceptRequest(otherId: string): Promise<void> {
  const myId = await currentUserId();
  if (myId === null) {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    throw new FriendsError("That didn't save. Give it another go in a moment.");
  }
 ...
 * @throws {FriendsError} With copy that's ready to render.
 */
export async function removeEdge(otherId: string): Promise<void> {
  const myId = await currentUserId();
  if (myId === null) {
    throw new FriendsError("You're signed out. Sign in and try that again.");
  }
