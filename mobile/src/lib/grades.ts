 ...
 *      deterministic and unit-testable on plain object literals, and takes
 *      whatever shape the screen already has in state, so optimistic local
 *      rows work exactly like fetched ones.
 *
...
/** Longest a category name may be, after trimming. Cap your TextInput here. */
...
/**
 * The caller's own id, read from the persisted session (no network round
 * trip). Every insert stamps it, because the RLS write policies check
 * `user_id = auth.uid()` and a mismatched id fails the policy, not a
 * constraint, which would surface as a confusing permissions error.
 */
 ...
 * fails on the phone rather than as a constraint violation.
 *
